...
ms:mtpsurl: https://technet.microsoft.com/es-es/library/Gg399044(v=OCS.15)
ms:contentKeyID: 48277038
ms.date: 01/07/2017
...
Installs one or more Lync Server databases. Este cmdlet se presentó en Lync Server 2010.
...
The command shown in Example 2 installs the Almacén de administración central on the pool atl-sql-001.litwareinc.com. The database will be installed in the rtc instance, and make use of the folder G:\\CSDB.
...
Lync Server makes extensive use of SQL Server databases, ranging from the Almacén de administración central to the Base de datos de archivado. As a general rule, these databases are set up at the same time you install Lync Server or at the same time you install a Lync Server role (such as Servidor de supervisión) that requires a database back end. After installation has taken place these databases typically will not need to be reinstalled or moved to new locations.
...
<td><p>If this parameter is included, the <strong>Install-CsDatabase</strong> cmdlet will use the SqlServerFqdn parameter to install the Almacén de administración central on the specified computer. This parameter is typically used only by Generador de topologías, and is generally called just once, during initial setup.</p></td>
</tr>
<tr class="even">
...
<td><p>Optional</p></td>
<td><p>System.Management.Automation.SwitchParameter</p></td>
<td><p>If present, additional database roles will be collocated with the Almacén de administración central.</p></td>
</tr>
<tr class="odd">
