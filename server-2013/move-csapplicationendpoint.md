...
ms:mtpsurl: https://technet.microsoft.com/es-es/library/Gg398188(v=OCS.15)
ms:contentKeyID: 48274452
ms.date: 01/07/2017
...
Moves an endpoint to a different Registrar pool. Este cmdlet se presentó en Lync Server 2010.
...
This cmdlet moves an existing endpoint contact object in Servicios de dominio de Active Directory from a Microsoft Office Communications Server 2007 R2 application pool to a Lync Server application pool, or from one Lync Server application pool to another. The application associated with the given endpoint must exist on the target pool. To determine which application is associated with the endpoint, run the **Get-CsTrustedApplicationEndpoint** cmdlet or the **Get-CsDialInConferencingAccessNumber** cmdlet.
